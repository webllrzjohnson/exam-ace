...
export default function QuizPlayer() {
  ...
  const mode = (location.state as any)?.mode || 'practice';
  ...
  const handleFinish = useCallback(() => {
    ...
    navigate(`/quiz/${quiz.id}/results`, {
      state: { answers: finalAnswers, timeTaken, mode },
    });
  }, [answers, fillAnswer, selectedOptions, question, navigate, quiz, startTime, mode, finished]);
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className={`w-7 h-7 rounded-full border-2 flex items-center justify-center text-xs font-bold shrink-0 ${
                          ...
                          {showFeedback && isRight ? <CheckCircle className="w-4 h-4" /> : showFeedback && isWrong ? <XCircle className="w-4 h-4" /> : String.fromCharCode(65 + i)}
                        </span>
                        {opt}
                      </div>
                    </button>
